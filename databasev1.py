...
def init_db():
    ...
    with get_db() as conn:
        ...
        # ── Migration: add 'role' column to existing DBs that predate this change ──
        existing_cols = [
...
def get_all_analyses() -> list:
    """Return all analyses across all users, with username (admin only)."""
    with get_db() as conn:
        rows = conn.execute(
            """SELECT a.*, u.username,
                      (SELECT 1 FROM reports r WHERE r.analysis_id = a.id) AS has_report
               FROM analyses a
...
def admin_delete_analysis(analysis_id: int) -> bool:
    """Delete any analysis regardless of owner (admin only). Returns True if deleted."""
    with get_db() as conn:
        cur = conn.execute(
...
def get_global_stats() -> dict:
    ...
    with get_db() as conn:
        ...
        if not row:
            return {"total_users": 0, "total_analyses": 0, "real_count": 0, "fake_count": 0}
        d = dict(row)
        for k in d:
            d[k] = d[k] or 0
        return d